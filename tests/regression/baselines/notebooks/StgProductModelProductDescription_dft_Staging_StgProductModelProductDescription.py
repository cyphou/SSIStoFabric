# Fabric Notebook
# Migrated from SSIS Package: StgProductModelProductDescription
# Task: dft_Staging_StgProductModelProductDescription
# Task Type: DATA_FLOW
...
# Source: ole_src_SalesLT_ProductModelProductDescription
# Uses Fabric connection (same as pipeline externalReferences)
df_source_ole_src_saleslt_productmodelproductdescription = spark.read.format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_Source")) \
    .option("query", """SELECT        ProductModelID, ProductDescriptionID, Culture, rowguid, ModifiedDate
FROM            SalesLT.ProductModelProductDescription
ORDER BY ProductModelID""") \
    .load()
logger.info(f"Read {{count}} rows from ole_src_SalesLT_ProductModelProductDescription", count=df_source_ole_src_saleslt_productmodelproductdescription.count())

df = df_source_ole_src_saleslt_productmodelproductdescription

# === Transformations ===

# Data Conversion: dcnv_Unicode_Strings
# TODO: Cast columns to target data types
# df = df.withColumn("Culture", df["Culture"].cast("str"))
...
# Destination: ole_dst_Staging_StgProductModelProductDescription -> Table: [Staging].[StgProductModelProductDescription]
# Connection: cmgr_DW (OLEDB → Fabric Connection)
df.write \
    .mode("append") \
    .format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_DW")) \
    .option("dbtable", "[Staging].[StgProductModelProductDescription]") \
    .save()
logger.info("Data written to [Staging].[StgProductModelProductDescription] via Fabric connection")

# === Completion ===
logger.info("Notebook completed: dft_Staging_StgProductModelProductDescription")
print("Migration notebook execution complete: dft_Staging_StgProductModelProductDescription")
