# Fabric Notebook
# Migrated from SSIS Package: StgProductModel
# Task: dft_Staging_StgProductModel
# Task Type: DATA_FLOW
...
# Source: ole_src_SalesLT_ProductModel
# Uses Fabric connection (same as pipeline externalReferences)
df_source_ole_src_saleslt_productmodel = spark.read.format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_Source")) \
    .option("query", """SELECT        ProductModelID, Name, CatalogDescription, rowguid, ModifiedDate
FROM            SalesLT.ProductModel
ORDER BY ProductModelID""") \
    .load()
logger.info(f"Read {{count}} rows from ole_src_SalesLT_ProductModel", count=df_source_ole_src_saleslt_productmodel.count())

df = df_source_ole_src_saleslt_productmodel
...
# Destination: ole_dst_Staging_StgProductModel -> Table: [Staging].[StgProductModel]
# Connection: cmgr_DW (OLEDB → Fabric Connection)
df.write \
    .mode("append") \
    .format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_DW")) \
    .option("dbtable", "[Staging].[StgProductModel]") \
    .save()
logger.info("Data written to [Staging].[StgProductModel] via Fabric connection")

# === Completion ===
logger.info("Notebook completed: dft_Staging_StgProductModel")
print("Migration notebook execution complete: dft_Staging_StgProductModel")
