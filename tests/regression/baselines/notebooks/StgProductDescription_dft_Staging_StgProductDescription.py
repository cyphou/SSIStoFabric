# Fabric Notebook
# Migrated from SSIS Package: StgProductDescription
# Task: dft_Staging_StgProductDescription
# Task Type: DATA_FLOW
...
# Source: ole_src_SalesLT_ProductDescription
# Uses Fabric connection (same as pipeline externalReferences)
df_source_ole_src_saleslt_productdescription = spark.read.format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_Source")) \
    .option("query", """SELECT        p.ProductDescriptionID, p.Description, p.rowguid, p.ModifiedDate
FROM            SalesLT.ProductDescription AS p INNER JOIN
                         SalesLT.ProductModelProductDescription AS pm ON p.ProductDescriptionID = pm.ProductDescriptionID
WHERE        (pm.Culture IN (N'en', N'fr'))
ORDER BY p.ProductDescriptionID""") \
    .load()
logger.info(f"Read {{count}} rows from ole_src_SalesLT_ProductDescription", count=df_source_ole_src_saleslt_productdescription.count())

df = df_source_ole_src_saleslt_productdescription

# === Transformations ===

# Data Conversion: dcnv_Unicode_Strings
# TODO: Cast columns to target data types
# df = df.withColumn("Description", df["Description"].cast("str"))
...
# Destination: ole_dst_Staging_StgProductDescription -> Table: [Staging].[StgProductDescription]
# Connection: cmgr_DW (OLEDB → Fabric Connection)
df.write \
    .mode("append") \
    .format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_DW")) \
    .option("dbtable", "[Staging].[StgProductDescription]") \
    .save()
logger.info("Data written to [Staging].[StgProductDescription] via Fabric connection")

# === Completion ===
logger.info("Notebook completed: dft_Staging_StgProductDescription")
print("Migration notebook execution complete: dft_Staging_StgProductDescription")
