# Fabric Notebook
# Migrated from SSIS Package: StgSalesOrderHeader
# Task: dft_Staging_StgSalesOrderHeader
# Task Type: DATA_FLOW
...
# Source: ole_src_SalesLT_SalesOrderHeader
# Uses Fabric connection (same as pipeline externalReferences)
df_source_ole_src_saleslt_salesorderheader = spark.read.format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_Source")) \
    .option("query", """SELECT        SalesOrderID, RevisionNumber, OrderDate, DueDate, ShipDate, Status, OnlineOrderFlag, SalesOrderNumber, PurchaseOrderNumber, AccountNumber, CustomerID, 
                         ShipToAddressID, BillToAddressID, ShipMethod, CreditCardApprovalCode, SubTotal, TaxAmt, Freight, TotalDue, LEFT(CAST(Comment AS VARCHAR(8000)), 128) AS Comment, rowguid, ModifiedDate
...
# df = df.withColumn("SalesOrderNumber", df["SalesOrderNumber"].cast("str"))
# df = df.withColumn("PurchaseOrderNumber", df["PurchaseOrderNumber"].cast("str"))
# df = df.withColumn("AccountNumber", df["AccountNumber"].cast("str"))
# df = df.withColumn("ShipMethod", df["ShipMethod"].cast("str"))
...
# Destination: ole_dst_Staging_StgSalesOrderHeader -> Table: [Staging].[StgSalesOrderHeader]
# Connection: cmgr_DW (OLEDB → Fabric Connection)
df.write \
    .mode("append") \
    .format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_DW")) \
    .option("dbtable", "[Staging].[StgSalesOrderHeader]") \
    .save()
logger.info("Data written to [Staging].[StgSalesOrderHeader] via Fabric connection")

# === Completion ===
logger.info("Notebook completed: dft_Staging_StgSalesOrderHeader")
print("Migration notebook execution complete: dft_Staging_StgSalesOrderHeader")
