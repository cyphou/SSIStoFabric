# Fabric Notebook
# Migrated from SSIS Package: StgSalesOrderDetail
# Task: dft_Staging_StgSalesOrderDetail
# Task Type: DATA_FLOW
...
# Source: ole_src_SalesLT_SalesOrderDetail
# Uses Fabric connection (same as pipeline externalReferences)
df_source_ole_src_saleslt_salesorderdetail = spark.read.format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_Source")) \
    .option("query", """SELECT        SalesOrderID, SalesOrderDetailID, OrderQty, ProductID, UnitPrice, UnitPriceDiscount, LineTotal, rowguid, ModifiedDate
FROM            SalesLT.SalesOrderDetail
ORDER BY  SalesOrderID, SalesOrderDetailID, ProductID""") \
    .load()
logger.info(f"Read {{count}} rows from ole_src_SalesLT_SalesOrderDetail", count=df_source_ole_src_saleslt_salesorderdetail.count())

df = df_source_ole_src_saleslt_salesorderdetail
...
# Destination: ole_dst_Staging_StgSalesOrderDetail -> Table: [Staging].[StgSalesOrderDetail]
# Connection: cmgr_DW (OLEDB → Fabric Connection)
df.write \
    .mode("append") \
    .format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_DW")) \
    .option("dbtable", "[Staging].[StgSalesOrderDetail]") \
    .save()
logger.info("Data written to [Staging].[StgSalesOrderDetail] via Fabric connection")

# === Completion ===
logger.info("Notebook completed: dft_Staging_StgSalesOrderDetail")
print("Migration notebook execution complete: dft_Staging_StgSalesOrderDetail")
