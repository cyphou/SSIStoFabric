# Fabric Notebook
# Migrated from SSIS Package: StgProductCategory
# Task: dft_Staging_StgProductCategory
# Task Type: DATA_FLOW
...
# Source: ole_src_SalesLT_ProductCategory
# Uses Fabric connection (same as pipeline externalReferences)
df_source_ole_src_saleslt_productcategory = spark.read.format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_Source")) \
    .option("query", """SELECT        ProductCategoryID, ParentProductCategoryID, Name, rowguid, ModifiedDate
FROM            SalesLT.ProductCategory
ORDER BY ProductCategoryID""") \
    .load()
logger.info(f"Read {{count}} rows from ole_src_SalesLT_ProductCategory", count=df_source_ole_src_saleslt_productcategory.count())

df = df_source_ole_src_saleslt_productcategory

# === Transformations ===

# Data Conversion: dcnv_Unicode_Strings
# TODO: Cast columns to target data types
# df = df.withColumn("Name", df["Name"].cast("str"))
...
# Destination: ole_dst_Staging_StgProductCategory -> Table: [Staging].[StgProductCategory]
# Connection: cmgr_DW (OLEDB → Fabric Connection)
df.write \
    .mode("append") \
    .format("jdbc") \
    .option("url", _jdbc_url_for("cmgr_DW")) \
    .option("dbtable", "[Staging].[StgProductCategory]") \
    .save()
logger.info("Data written to [Staging].[StgProductCategory] via Fabric connection")

# === Completion ===
logger.info("Notebook completed: dft_Staging_StgProductCategory")
print("Migration notebook execution complete: dft_Staging_StgProductCategory")
