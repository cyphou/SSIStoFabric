...
<!-- Column Lineage Sankey Diagram (D3.js) -->
{column_lineage_section}
...
class ReportGenerator:
    ...
    def _render(self, report: dict[str, Any]) -> str:
        ...
        return _HTML_TEMPLATE.format(
            ...
            column_lineage_section=self._render_column_lineage(report),
        )

    def _render_column_lineage(self, report: dict[str, Any]) -> str:
        """Render a D3.js Sankey diagram for column lineage if data is present."""
        col_lineage = report.get("column_lineage")
        if not col_lineage:
            return ""

        sankey_data = col_lineage.get("sankey", {})
        ...
        edge_count = col_lineage.get("summary", {}).get("total_edges", len(links))
        col_count = col_lineage.get("summary", {}).get("total_columns", len(nodes))

        return (
            '<section><h2>Column-Level Lineage</h2>'
            f'<p>{col_count} columns, {edge_count} edges</p>'
            f'<div id="sankey" style="width:100%;min-height:400px;overflow-x:auto;"></div>'
            ...
            'const w=document.getElementById("sankey").clientWidth||900,h=Math.max(400,nodes.length*20);'
            'const svg=d3.select("#sankey").append("svg").attr("width",w).attr("height",h);'
            ...
            '.attr("stroke",d=>color(d.transform||"default"))'
            '.attr("stroke-width",d=>Math.max(1,d.width))'
            '.append("title").text(d=>'
            '`${graph.nodes[d.source.index||d.source].name}'
            ' \u2192 ${graph.nodes[d.target.index||d.target].name} (${d.transform})`);'
